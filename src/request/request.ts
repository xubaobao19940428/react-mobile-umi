...
const BASE = baseUrl['api'] + 'ball_front/'
...
function request(params: requestParams) {
    ...
    const headersConfig: headersConfigParams = {
        'X-Version-Code': localStorage.getItem('versionCode') || (process.env.UMI_ENV == 'pre' ? '1.0.0.pre' : '1.0.0'),
    ...
    const urlConfig = {
        ...
        version: localStorage.getItem('versionCode') ||
            (process.env.UMI_ENV == 'pre' ? '1.0.0.pre' : '1.0.0'),
            ...
            .then((res) => {
                ...
                if (
                    ...
                    if (result.resultCode.code == '401') {
                        ...
                        Toast.show({
                            content: result.resultCode.message,
                            afterClose: () => {
                                console.log('after')
                            },
                        })
